...
const trackBackgroundStyle = (() => {
  ...
  const bgDayNight = `linear-gradient(
        to bottom,
        #dddddd 0px, #dddddd ${activeStart}px,
        #ffffff ${activeStart}px, #ffffff ${activeEnd}px,
        #dddddd ${activeEnd}px, #dddddd ${dayH}px
    )`;
  // Layer 2: Slot Snap Lines (Middle, Faint)
  const bgSlotLines = `linear-gradient(to bottom, rgba(0,0,0,0.06) 1px, transparent 1px)`;

  return {
    backgroundImage: `${bgSlotLines}, ${bgDayNight}`,
    backgroundSize: `100% ${slotH}px, 100% ${dayH}px`,
...
export function Scheduler() {
  ...
  // Lock body scroll when scheduler is open
  useEffect(() => {
    document.body.classList.add("scheduler-open");
    return () => document.body.classList.remove("scheduler-open");
  }, []);
      ...
      <div class={styles.container}>
        <div class={styles.header}>
          ...
          <div class={styles.controls}>
            <button class={styles.cancel} onClick={discardChanges}>
              Cancel
            </button>
            <button class={styles.save} onClick={saveChanges}>
              Confirm Changes
            </button>
          </div>
        </div>
    ...
    >
      ...
      {talks.map((t) => {
        ...
        return (
          <div
            key={t.id}
            class={`${styles.draggableItem} ${t.highlight ? styles.highlight : ""}`}
            draggable
...
function Timeline() {
  ...
  const handleDragOver = (e: DragEvent, venueId: number) => {
    ...
    const totalMinutes = getSnapMinutes(
      e.clientY,
    ...
    const timeLabel = `Day ${dayIdx + 1}, ${Math.floor(minsIntoDay / 60)
      .toString()
      .padStart(2, "0")}:${(minsIntoDay % 60).toString().padStart(2, "0")}`;
  ...
  const handleDrop = (e: DragEvent, venueId: number) => {
    ...
    updateDraftTalk(talkId, {
      location: venueId,
      scheduled_at: toSystemTime(
        new Date((schedulerConfig.startEpoch + totalMinutes * 60) * 1000),
      ),
    });
    draggingTalkId.value = null;
  };
  ...
  const showRedLine =
    nowOffset >= 0 && nowOffset < schedulerConfig.daysToShow * 86400;

  return (
    <div class={styles.timelineContainer}>
      <div class={styles.timelineHeader} style={{ paddingLeft: "60px" }}>
        {venues.value.map((v) => (
          <div key={v.id} class={styles.venueHeader}>
            {v.name}
            <span
              style={{ marginLeft: "6px", fontSize: "0.8rem" }}
              title={v.live_stream ? "Stream available" : "No stream"}
            >
              {v.live_stream ? "📺" : "🚫"}
            </span>
          </div>
      ...
      <div class={styles.timelineBody} ref={scrollRef}>
        <div class={styles.timeAxis} style={{ height: `${totalHeight}px` }}>
          {Array.from({
            length: (schedulerConfig.daysToShow * MINS_PER_DAY) / 60,
          }).map((_, i) => (
            <div
              key={i}
              class={styles.timeLabel}
        ...
        <div class={styles.tracksContainer} style={trackBackgroundStyle}>
          {/* Hour Grid Lines (Rendered as DOM elements for precise alignment with Time Axis) */}
          {Array.from({
            length: (schedulerConfig.daysToShow * MINS_PER_DAY) / 60,
          }).map((_, i) => (
            <div
          ...
          {Array.from({ length: schedulerConfig.daysToShow }).map((_, i) => {
            const date = new Date(
              (schedulerConfig.startEpoch + i * 86400) * 1000,
            );
            return (
              <div
                key={i}
                class={styles.daySeparator}
                style={{ top: `${i * MINS_PER_DAY * ZOOM}px` }}
              >
          ...
          {showRedLine && (
            <div
              class={styles.currentTimeLine}
              style={{ top: `${(nowOffset / 60) * ZOOM}px` }}
            >
          ...
          {venues.value.map((venue) => (
            ...
            >
              {shadow?.venueId === venue.id && (
                <div
                  class={styles.shadowEvent}
                  style={{
                    top: `${shadow.top}px`,
                    height: `${shadow.height - 1}px`,
                  }}
                >
                  <span
                    style={{
              ...
              {activeTalks.value
                ...
                .map((t) => (
                  <ScheduledEvent key={t.id} talk={t} />
                ))}
            </div>
          ))}
        </div>
...
function ScheduledEvent({ talk }: { talk: Talk }) {
  const startMins =
    (talk.scheduled_at!.secs_since_epoch - schedulerConfig.startEpoch) / 60;
  ...
  const handleResizeStart = (e: PointerEvent) => {
    ...
    const onMove = (evt: PointerEvent) => {
      const rawH = Math.max(
        SLOT_MINUTES * ZOOM,
        startHeight + (evt.pageY - startY),
      );
      const snappedMins = Math.round(rawH / ZOOM / SLOT_MINUTES) * SLOT_MINUTES;
      updateDraftTalk(talk.id, {
        duration: { secs: snappedMins * 60, nanos: 0 },
      });
    };
    const onUp = () => {
  ...
  return (
    <div
      class={`${styles.event} ${talk.highlight ? styles.highlight : ""}`}
      style={{
        top: `${top}px`,
        height: `${height - 1}px`,
        opacity: draggingTalkId.value === talk.id ? 0.5 : 1,
      }}
      title={`${talk.title}\nBy ${authorName}\n${timeStr}`}
      draggable
